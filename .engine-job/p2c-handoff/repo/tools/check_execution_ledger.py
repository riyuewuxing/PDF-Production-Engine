...
def validate_attempt_protocol(events: list[dict], contract: dict) -> list[str]:
    """Validate ordered attempt facts while permitting one in-progress final attempt.

    HUMAN review is valid for a wholly nonvisual job after machine verification; once
    this job has any FINAL_RENDER_COMPLETED event, every reviewed attempt must carry its
    own final render with the same attempt_id. Only rendered reviews count as full
    process-evaluation generation/render/HUMAN-review cycles.
    """
    ...
    visual_mode_observed = any(
        "FINAL_RENDER_COMPLETED" in [typ for _, typ in grouped.get(attempt, [])]
        for attempt in order
    )

    for attempt in order:
        sequence = grouped.get(attempt, [])
        types = [typ for _, typ in sequence]
        label = f"attempt {attempt}"
        if not types:
            continue
        if types[0] != "BUILD_STARTED":
            errors.append(f"{label}: first attempt event must be BUILD_STARTED, got {types[0]}")
        for typ in set(types):
            if types.count(typ) > 1:
                errors.append(f"{label}: duplicate attempt stage forbidden: {typ}")

        build_results = [x for x in ("BUILD_COMPLETED", "BUILD_FAILED") if x in types]
        if len(build_results) > 1:
            errors.append(f"{label}: BUILD_COMPLETED and BUILD_FAILED are mutually exclusive")
        machine_results = [x for x in ("MACHINE_VERIFICATION_PASSED", "MACHINE_VERIFICATION_FAILED") if x in types]
        if len(machine_results) > 1:
            errors.append(f"{label}: MACHINE_VERIFICATION_PASSED and MACHINE_VERIFICATION_FAILED are mutually exclusive")
        review_results = [x for x in ("HUMAN_REVIEW_ACCEPTED", "HUMAN_REVIEW_REJECTED") if x in types]
        if len(review_results) > 1:
            errors.append(f"{label}: HUMAN review result must be exactly one outcome per reviewed attempt")
        ...
        if any(x in types for x in ("MACHINE_VERIFICATION_PASSED", "MACHINE_VERIFICATION_FAILED")):
            if "BUILD_COMPLETED" not in types:
                errors.append(f"{label}: machine verification requires BUILD_COMPLETED")
            elif pos["BUILD_COMPLETED"] > min(pos[x] for x in machine_results):
                errors.append(f"{label}: machine verification must occur after BUILD_COMPLETED")
        if "FINAL_RENDER_COMPLETED" in types:
            if "MACHINE_VERIFICATION_PASSED" not in types:
                errors.append(f"{label}: FINAL_RENDER_COMPLETED requires MACHINE_VERIFICATION_PASSED")
            elif pos["MACHINE_VERIFICATION_PASSED"] > pos["FINAL_RENDER_COMPLETED"]:
                errors.append(f"{label}: FINAL_RENDER_COMPLETED must occur after MACHINE_VERIFICATION_PASSED")
        if review_results:
            review = review_results[0]
            if "MACHINE_VERIFICATION_PASSED" not in types:
                errors.append(f"{label}: {review} requires MACHINE_VERIFICATION_PASSED")
            elif pos["MACHINE_VERIFICATION_PASSED"] > pos[review]:
                errors.append(f"{label}: HUMAN review must occur after MACHINE_VERIFICATION_PASSED")
            ...
            if "FINAL_RENDER_COMPLETED" in types and pos["FINAL_RENDER_COMPLETED"] > pos[review]:
                errors.append(f"{label}: visual HUMAN review must occur after FINAL_RENDER_COMPLETED")

        terminal = _attempt_terminal(types)
        if terminal is not None:
            terminal_pos = pos[terminal]
            if terminal_pos != len(types) - 1:
                errors.append(f"{label}: no attempt event may follow terminal result {terminal}")
        if "BUILD_FAILED" in types:
            forbidden = set(types) & {
                "BUILD_COMPLETED", "MACHINE_VERIFICATION_PASSED", "MACHINE_VERIFICATION_FAILED",
                "FINAL_RENDER_COMPLETED", "HUMAN_REVIEW_ACCEPTED", "HUMAN_REVIEW_REJECTED",
            }
            if forbidden:
                errors.append(f"{label}: BUILD_FAILED attempt cannot continue: {sorted(forbidden)}")
        if "MACHINE_VERIFICATION_FAILED" in types:
            ...
            if forbidden:
                errors.append(f"{label}: MACHINE_VERIFICATION_FAILED attempt cannot continue: {sorted(forbidden)}")
    return errors
...
def validate(data: dict, root: Path = ROOT, contract_path: Path | None = None) -> list[str]:
    ...
    if data.get("contract") != CONTRACT.as_posix():
        errors.append(f"ledger: contract must be {CONTRACT.as_posix()}")
    if not isinstance(data.get("job_id"), str) or not data["job_id"].strip():
        errors.append("ledger: job_id is required")
    events = data.get("events")
    if not isinstance(events, list):
        return errors + ["ledger: events must be a list"]
    if not events:
        errors.append("ledger: a real ledger must contain JOB_CREATED")
    allowed = set(contract.get("event_types") or [])
    attempt_types = set(contract.get("attempt_event_types") or [])
    ...
    for idx, event in enumerate(events):
        ...
        if event.get("seq") != expected_seq:
            errors.append(f"{label}: seq must be {expected_seq}")
        expected_seq += 1
        ...
        if not parse_iso8601(event.get("occurred_at")):
            errors.append(f"{label}: occurred_at must be ISO-8601")
        if not isinstance(event.get("actor"), str) or not event.get("actor"):
            errors.append(f"{label}: actor is required")
        if typ in attempt_types and (not isinstance(event.get("attempt_id"), str) or not event.get("attempt_id", "").strip()):
            errors.append(f"{label}: attempt_id is required for {typ}")
        bindings = event.get("bindings") or []
        if not isinstance(bindings, list):
            errors.append(f"{label}: bindings must be a list")
        else:
            for bidx, binding in enumerate(bindings):
                errors.extend(validate_binding(root, binding, f"{label}.bindings[{bidx}]"))
    ...
    lifecycle_state, lifecycle_errors = project_lifecycle(normalized_events, contract)
    errors.extend(lifecycle_errors)
    actual = derive(normalized_events)
    declared = data.get("derived")
    if declared is not None:
        if not isinstance(declared, dict):
            errors.append("ledger: derived must be a mapping when present")
        else:
            for key, value in actual.items():
                if declared.get(key) != value:
                    errors.append(f"ledger: derived.{key} must equal checker-derived {value}")
            if declared.get("artifact_state") != lifecycle_state:
                errors.append(f"ledger: derived.artifact_state must equal checker-projected {lifecycle_state}")
    return errors
...
def selftest() -> int:
    with tempfile.TemporaryDirectory(prefix="qz-ledger-") as tmp:
        ...
        if found:
            print("FAIL: good ledger rejected", found)
            return 1
        if derive(good["events"])["build_count"] != 2:
            print("FAIL: failed build attempt was not counted")
            return 1
        bad = yaml.safe_load(yaml.safe_dump(good)); bad["derived"]["generation_review_cycles"] = 0
        if not validate(bad, root):
            print("FAIL: derived summary drift escaped"); return 1
        bad2 = yaml.safe_load(yaml.safe_dump(good)); bad2["events"][1]["type"] = "BUILT"
        if not validate(bad2, root):
            print("FAIL: lifecycle skip escaped"); return 1
        bad3 = yaml.safe_load(yaml.safe_dump(good)); bad3["events"][1]["seq"] = 99
        if not validate(bad3, root):
            print("FAIL: sequence drift escaped"); return 1
        bad_review = yaml.safe_load(yaml.safe_dump(good)); bad_review["events"][4]["type"] = "MACHINE_VERIFICATION_FAILED"
        if not any("cannot continue" in x for x in validate(bad_review, root)):
            print("FAIL: review after machine failure escaped"); return 1
        interleaved = yaml.safe_load(yaml.safe_dump(good))
        interleaved["events"].insert(8, {"seq": 9, "event_id": "ix", "occurred_at": "2026-01-01T00:00:08Z", "type": "FINAL_RENDER_COMPLETED", "actor": "Engine", "attempt_id": "a1"})
        for i, event in enumerate(interleaved["events"], 1): event["seq"] = i
        if not any("interleaved" in x or "after a later attempt" in x for x in validate(interleaved, root)):
            print("FAIL: interleaved attempts escaped"); return 1
        accepted_then_retry = yaml.safe_load(yaml.safe_dump(good)); accepted_then_retry["events"][6]["type"] = "HUMAN_REVIEW_ACCEPTED"
        if not any("must terminate the job attempt sequence" in x for x in validate(accepted_then_retry, root)):
            print("FAIL: retry after HUMAN acceptance escaped"); return 1
        nonvisual = {
            "contract": CONTRACT.as_posix(), "job_id": "nonvisual",
            "events": [
                {"seq": 1, "event_id": "n1", "occurred_at": "2026-01-01T00:00:00Z", "type": "JOB_CREATED", "actor": "ChatGPT"},
                {"seq": 2, "event_id": "n2", "occurred_at": "2026-01-01T00:00:01Z", "type": "BUILD_STARTED", "actor": "Engine", "attempt_id": "n-a1"},
                {"seq": 3, "event_id": "n3", "occurred_at": "2026-01-01T00:00:02Z", "type": "BUILD_COMPLETED", "actor": "Engine", "attempt_id": "n-a1"},
                {"seq": 4, "event_id": "n4", "occurred_at": "2026-01-01T00:00:03Z", "type": "MACHINE_VERIFICATION_PASSED", "actor": "Engine", "attempt_id": "n-a1"},
                {"seq": 5, "event_id": "n5", "occurred_at": "2026-01-01T00:00:04Z", "type": "HUMAN_REVIEW_ACCEPTED", "actor": "ChatGPT", "attempt_id": "n-a1"},
            ],
        }
        if validate(nonvisual, root):
            print("FAIL: valid nonvisual human review rejected"); return 1
        if derive(nonvisual["events"])["generation_review_cycles"] != 0:
            print("FAIL: nonvisual review incorrectly counted as full render cycle"); return 1
        mixed_visual = yaml.safe_load(yaml.safe_dump(good))
        mixed_visual["events"][8] = {"seq": 9, "event_id": "e9", "occurred_at": "2026-01-01T00:00:08Z", "type": "BUILD_COMPLETED", "actor": "Engine", "attempt_id": "a2"}
        mixed_visual["events"].extend([
            {"seq": 10, "event_id": "e10", "occurred_at": "2026-01-01T00:00:09Z", "type": "MACHINE_VERIFICATION_PASSED", "actor": "Engine", "attempt_id": "a2"},
            {"seq": 11, "event_id": "e11", "occurred_at": "2026-01-01T00:00:10Z", "type": "HUMAN_REVIEW_ACCEPTED", "actor": "ChatGPT", "attempt_id": "a2"},
        ])
        if not any("reviewed visual job attempt requires its own FINAL_RENDER_COMPLETED" in x for x in validate(mixed_visual, root)):
            print("FAIL: visual retry reused prior attempt render"); return 1
        transient = {
            "contract": CONTRACT.as_posix(), "job_id": "transient",
            "events": [
        ...
        if validate(transient, root):
            print("FAIL: one in-progress final attempt should be valid"); return 1
    print("PASS: execution ledger selftest")
    return 0
...
def main() -> int:
    ...
    parser.add_argument("--report", action="store_true", help="print checker-derived metrics after a valid ledger")
    args = parser.parse_args()
    if args.selftest:
        return selftest()
    if not args.ledger:
        parser.error("--ledger or --selftest is required")
    ...
    if args.report:
        contract = load_yaml(args.root.resolve() / CONTRACT)
        state, _ = project_lifecycle([x for x in data.get("events") or [] if isinstance(x, dict)], contract)
