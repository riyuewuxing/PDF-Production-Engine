...
CONTENT_CONTRACT_ID = "teaching-demo-content"
...
def validate(manifest_raw: str | None = None, *, root: Path = ROOT) -> list[str]:
    ...
    if cfg.get("content_contract") != CONTENT_CONTRACT_ID:
        errors.append("CASE_CONTENT_CONTRACT_DRIFT")
    ...
    if not isinstance(printed, list) or not printed or not all(isinstance(x, int) and x > 0 for x in printed):
        errors.append("CASE_PRINTED_PAGES_MUST_BE_NONEMPTY_POSITIVE_INTS")
    mode = sp.get("mode")
    if mode not in {"remote-pdf", "repository-pdf"}:
        errors.append(f"CASE_FORMAL_SOURCE_MODE_INVALID: {mode!r}")
    if not isinstance(sp.get("blob_sha"), str) or not HEX40.fullmatch(sp.get("blob_sha")):
        errors.append("CASE_SOURCE_LOCK_INVALID: blob_sha")
    ...
    if mode == "remote-pdf":
        ...
        if not isinstance(sp.get("ref"), str) or not HEX40.fullmatch(sp.get("ref")):
            errors.append("CASE_SOURCE_LOCK_INVALID: ref")
    elif mode == "repository-pdf":
        raw_file = sp.get("file")
        if not isinstance(raw_file, str) or not raw_file:
            errors.append("CASE_REPOSITORY_SOURCE_FILE_MISSING")
        else:
            try:
                source_file = safe_repo_path(root, raw_file, must_exist=True)
                if not source_file.is_file():
                    errors.append("CASE_REPOSITORY_SOURCE_NOT_FILE")
                else:
                    if source_file.stat().st_size != sp.get("size_bytes"):
                        errors.append("CASE_REPOSITORY_SOURCE_SIZE_DRIFT")
                    if isinstance(sp.get("blob_sha"), str) and HEX40.fullmatch(sp.get("blob_sha")):
                        actual_blob = _git_blob_sha(source_file)
                        if actual_blob != sp.get("blob_sha"):
                            errors.append("CASE_REPOSITORY_SOURCE_BLOB_DRIFT")
            except Exception as exc:
                errors.append(f"CASE_REPOSITORY_SOURCE_INVALID: {exc}")
        audit = sp.get("audit_locator") or {}
        if not isinstance(audit, dict):
            errors.append("CASE_SOURCE_AUDIT_LOCATOR_INVALID")
        else:
            for key in ("repo", "path"):
                if not isinstance(audit.get(key), str) or not audit.get(key):
                    errors.append(f"CASE_SOURCE_AUDIT_LOCK_MISSING: {key}")
            for key in ("ref", "blob_sha"):
                if not isinstance(audit.get(key), str) or not HEX40.fullmatch(audit.get(key)):
                    errors.append(f"CASE_SOURCE_AUDIT_LOCK_INVALID: {key}")
    physical = sp.get("pages")
    if not isinstance(physical, list) or not physical or not all(isinstance(x, int) and x > 0 for x in physical):
    ...
    if workspace is not None:
        ...
        if evidence_rel:
            try:
                ...
                locator_source = sp.get("audit_locator") if mode == "repository-pdf" else sp
                locator_source = locator_source if isinstance(locator_source, dict) else {}
                expected_locator = {
                    "repo": locator_source.get("repo"),
                    "ref": locator_source.get("ref"),
                    "path": locator_source.get("path"),
                    "blob_sha": locator_source.get("blob_sha"),
                    "pdf_pages": physical,
                }
