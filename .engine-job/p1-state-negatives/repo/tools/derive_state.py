...
def _validate_ledger_structure(
    ...
    *,
    ...
    for index, raw in enumerate(events, start=1):
        ...
        assertion_policy = contract.get("execution_assertions") or {}
        assertion = raw.get(str(assertion_policy.get("optional_field") or "assertion"))
        pass_values = set(assertion_policy.get("pass_values") or [])
        if assertion in pass_values and assertion_policy.get("pass_requires_execution_surface") is True:
            surface = raw.get("execution_surface")
            if not isinstance(surface, str) or not surface.strip():
                errors.append(f"{label}: PASS-class assertion requires non-empty execution_surface")
        if assertion == assertion_policy.get("blocked_value") and assertion_policy.get("blocked_requires_nonempty_blocked_by") is True:
            blocked_by = raw.get("blocked_by")
            if not isinstance(blocked_by, list) or not blocked_by or not all(isinstance(x, str) and x for x in blocked_by):
                errors.append(f"{label}: BLOCKED assertion requires non-empty blocked_by")
